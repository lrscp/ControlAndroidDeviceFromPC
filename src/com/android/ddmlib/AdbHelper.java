 ...
 * <p/>
 * {@link DebugBridgeServer} is the public API to connection to adb, while
...
final class AdbHelper {
    ...
    static final String DEFAULT_ENCODING = "utf-8"; //"ISO-8859-1"$NON-NLS-1$;
    ...
    public static SocketChannel createPassThroughConnection(InetSocketAddress adbSockAddr,
            Device device, int pid) throws TimeoutException, AdbCommandRejectedException,
            IOException {
    ...
    static byte[] formAdbRequest(String req) {
        ...
        try {
            ...
            //Log.e(null, resultStr + " len:" + resultStr.length());
            assert result.length == req.getBytes(DEFAULT_ENCODING).length + 4;
        ...
        try {
            while (readDiagString) {
                ...
                try {
                    ...
                    Log.w("ddms", "Expected digits, got '" + lenStr + "': " + lenBuf[0] + " "
                            + lenBuf[1] + " " + lenBuf[2] + " " + lenBuf[3]);
    ...
    static RawImage getFrameBuffer(InetSocketAddress adbSockAddr, Device device)
        ...
        try {
            ...
            Log.d("ddms", "image params: bpp=" + imageParams.bpp + ", size=" + imageParams.size
    ...
    static void executeRemoteCommand(InetSocketAddress adbSockAddr, String command, IDevice device,
            IShellOutputReceiver rcvr, int maxTimeToOutputResponse) throws TimeoutException,
            AdbCommandRejectedException, ShellCommandUnresponsiveException, IOException {
        Log.v("ddms", "execute: running " + command + " address:" + adbSockAddr);
     ...
     *            the socket address to connect to adb
     * @param device
     *            the device on which to do the port fowarding
     * @param localPortSpec
    ...
    public static void createForward(InetSocketAddress adbSockAddr, Device device,
            String localPortSpec, String remotePortSpec) throws TimeoutException,
     ...
     *            the socket address to connect to adb
     * @param device
     *            the device on which to remove the port fowarding
     * @param localPortSpec
    ...
    public static void removeForward(InetSocketAddress adbSockAddr, Device device,
        ...
        try {
            ...
            byte[] request = formAdbRequest(String.format("host-serial:%1$s:killforward:%2$s;%3$s", //$NON-NLS-1$
                    device.getSerialNumber(), localPortSpec, remotePortSpec));
    ...
    static boolean isOkay(byte[] reply) {
        return reply[0] == (byte) 'O' && reply[1] == (byte) 'K' && reply[2] == (byte) 'A'
                && reply[3] == (byte) 'Y';
    }
     ...
     *             in case of I/O error on the connection.
     */
    static void setDevice(SocketChannel adbChan, IDevice device) throws TimeoutException,
            AdbCommandRejectedException, IOException {
    ...
    public static String execRawCmd(InetSocketAddress adbSockAddr, Device device, String cmd)
            throws TimeoutException, AdbCommandRejectedException, IOException {
        ...
        try {
            ...
            write(adbChan, formAdbRequest(cmd));
            ...
            while (true) {
                ...
                if (count < 0) {
                    ...
                    // reset timeout
                    timeToResponseCount = 0;
                    sb.append(new String(Arrays.copyOfRange(data, 0, buf.position()), "utf-8"));
                    buf.rewind();
                    if (sb.length() > 1000000) {
                        Log.w("execRawCmd", "large string output, break");
                        break;
                    }
                }
            }
